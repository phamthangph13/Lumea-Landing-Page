...
export default {
  ...
  head() {
    const title = this.currentProject && this.currentProject.title ? this.currentProject.title.full : 'Trụ Cột Thương Hiệu'
    return {
      title: `LUMÉA | ${title}`,
      meta: [
        { hid: 'og:title', property: 'og:title', content: `LUMÉA | ${title}` },
      ]
    }
  },
  ...
  computed: {
    ...
    currentProject() {
      ...
      let index = projectResponse.findIndex((content) => content.id === this.$router.history.current.params.slug)
      if (index === -1) index = 0
      const currentProject = Object.assign({}, projectResponse[index])
